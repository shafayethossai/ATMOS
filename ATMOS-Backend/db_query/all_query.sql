...
-- 2. OTP Tokens Table
-- Handles both signup verification and forgot-password OTPs.
-- pending_name and pending_hash store new user data during signup
-- so the account can be created on the verify step.
...
-- Short-lived token issued after forgot-password OTP is verified.
-- Frontend sends it back alongside the new password.
...
-- Stores raw IoT sensor values and the AQI calculated by Go on insert.
-- o3 (ppb, 8-hour avg) and no2 (ppb) are included in AQI.
-- co2 (ppm) is a ventilation metric only — stored but excluded from AQI.
-- pm25_after and pm10_after are post-purification filter readings.
